  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-primary text-primary-foreground">
        <div className="container-page flex items-center justify-between py-1.5 sm:py-2 text-[10px] sm:text-xs font-body">
          <div className="flex items-center gap-2 sm:gap-4">
            <a href="tel:+919123506940" className="flex items-center gap-1 hover:opacity-80 transition-opacity">
              <Phone className="h-3 w-3 shrink-0" />
              <span className="hidden xs:inline">+91 91 2350 694 0</span>
      ...
      <nav className="bg-background border-b border-border">
        <div className="container-page flex items-center justify-between h-12 sm:h-14">
          {/* Mobile menu button */}
          <Button variant="ghost" size="icon" className="lg:hidden h-8 w-8 sm:h-9 sm:w-9" onClick={() => setMobileOpen(!mobileOpen)}>
          ...
          <Link to="/" className="absolute left-1/2 -translate-x-1/2 lg:static lg:translate-x-0">
            <img src={logo} alt="Habeeb's Paradise" className="h-7 sm:h-9 md:h-11 w-auto object-contain" />
          </Link>
          ...
          <div className="flex flex-col p-4 gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="text-sm font-body text-foreground hover:text-primary py-3 border-b border-border transition-colors"
                onClick={() => setMobileOpen(false)}
              >
                {link.name}
              </Link>
            ))}
            <Link to="/track-order" className="text-sm font-body text-foreground hover:text-primary py-3 border-b border-border" onClick={() => setMobileOpen(false)}>
              Track Order
            </Link>
            {isAdmin && (
              <Link to="/admin" className="text-sm font-body text-primary font-semibold hover:text-primary py-3 border-b border-border flex items-center gap-2" onClick={() => setMobileOpen(false)}>
